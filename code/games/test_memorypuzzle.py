...
def main():
    ...
    quitRectObj = quitSurfaceObj.get_rect(center=(WINDOWWIDTH - (XMARGIN/2), 35))
    quitButtonRect = quitRectObj.inflate(30, 16)
    pygame.draw.rect(DISPLAYSURF, LIGHTBGCOLOR, quitButtonRect, border_radius=8)
...
def revealBoxesAnimation(board, boxesToReveal):
    ...
    for coverage in range(BOXSIZE, 0, -REVEALSPEED):
        drawBoxCovers(board, boxesToReveal, coverage)
    drawBoxCovers(board, boxesToReveal, 0)

def coverBoxesAnimation(board, boxesToCover):
    # Do the "box cover" animation.
    for coverage in range(0, BOXSIZE, REVEALSPEED):
        drawBoxCovers(board, boxesToCover, coverage)
    drawBoxCovers(board, boxesToCover, BOXSIZE)
